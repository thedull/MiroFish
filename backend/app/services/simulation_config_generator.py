...
# Chinese daily schedule configuration (Beijing Time, UTC+8)
...
class AgentActivityConfig:
    ...
    # Influence weight (determines the probability that this agent's posts are seen by others)
...
class TimeSimulationConfig:
    ...
    # Peak hours (evening 19:00–22:00, when Chinese users are most active)
...
class PlatformConfig:
    ...
    # Echo chamber effect intensity (degree to which similar opinions cluster together)
...
class SimulationConfigGenerator:
    ...
    Uses LLM to analyze simulation requirements, document content, and knowledge
    graph entity information, then automatically generates an optimal simulation
    ...
    def _try_fix_config_json(self, content: str) -> Optional[Dict[str, Any]]:
        ...
        if json_match:
            ...
            # Remove newlines from within string values
...
- Infer the timezone and daily schedule habits of the target user group from the simulation scenario. The following is a reference example for UTC+8 (China Standard Time).
- Almost no activity from 0:00–5:00 (activity multiplier: 0.05)
- Gradually becoming active from 6:00–8:00 (activity multiplier: 0.4)
- Moderate activity during work hours 9:00–18:00 (activity multiplier: 0.7)
- Evening peak from 19:00–22:00 (activity multiplier: 1.5)
- Activity declining after 23:00 (activity multiplier: 0.5)
- General pattern: low late at night, gradually increasing in the morning, moderate during work hours, peak in the evening
- **Important**: The example values below are for reference only. You must adjust specific time ranges based on the nature of the event and characteristics of the participant group.
  - Example: Students may peak at 21:00–23:00; media outlets are active all day; official institutions are only active during business hours.
  - Example: Breaking news may spark discussion even late at night, so off_peak_hours can be shortened accordingly.
...
- peak_hours (int array): Peak hours — adjust based on the participant group for this event
- off_peak_hours (int array): Off-peak hours — typically late at night
...
- reasoning (string): Brief explanation of why this configuration was chosen"""
        ...
        system_prompt = "You are a social media simulation expert. Return pure JSON format. Time configuration must match the daily schedule habits of the target user group in the simulation scenario."
    ...
    def _get_default_time_config(self, num_entities: int) -> Dict[str, Any]:
        """Get default time configuration (based on Chinese daily schedule)"""
        return {
            "total_simulation_hours": 72,
...
**Important**: poster_type must be chosen from the "Available Entity Types" listed above so that initial posts can be assigned to appropriate agents. For example: official announcements should be posted by Official/University types, news by MediaOutlet, student perspectives by Student.
...
{{
    ...
    def _assign_initial_post_agents(
        ...
        Matches the most suitable agent_id to each post based on its poster_type
        """
...
- **Schedule must match the target user group**: The following is for reference (UTC+8); adjust based on the simulation scenario.
- **Official institutions** (University/GovernmentAgency): Low activity (0.1–0.3), active during business hours (9:00–17:00), slow response (60–240 min), high influence (2.5–3.0)
- **Media** (MediaOutlet): Moderate activity (0.4–0.6), active all day (8:00–23:00), fast response (5–30 min), high influence (2.0–2.5)
- **Individuals** (Student/Person/Alumni): High activity (0.6–0.9), mainly active in the evening (18:00–23:00), fast response (1–15 min), low influence (0.8–1.2)
...
{{
    "agent_configs": [
        ...
        system_prompt = "You are a social media behavior analysis expert. Return pure JSON. Configurations must match the daily schedule habits of the target user group in the simulation scenario."
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        """Rule-based generation of a single agent configuration (based on Chinese daily schedule)"""
