...
class OasisProfileGenerator:
    ...
    2. Generates highly detailed personas (including basic info, career history,
    ...
    # Group / institutional-type entities (require generating representative account personas)
    GROUP_ENTITY_TYPES = [
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Use Zep knowledge graph hybrid search to retrieve rich information about an entity.

        Zep does not have a built-in hybrid search endpoint, so edges and nodes are searched
        separately and the results are merged. Parallel requests are used for efficiency.
    ...
    def _is_group_entity(self, entity_type: str) -> bool:
        """Determine whether the entity is a group / institutional type"""
    ...
    def _fix_truncated_json(self, content: str) -> str:
        ...
        # Simple check: if the last quote is not followed by a comma or closing bracket,
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        if json_match:
            ...
            # 3. Handle newline characters within string values
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are an expert in generating social media user personas. Generate detailed, realistic personas for public opinion simulation that faithfully reproduce real-world situations as closely as possible. You must return valid JSON format; all string values must not contain unescaped newlines."
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for this entity, faithfully reproducing real-world situations as closely as possible.
...
- persona must be a continuous, cohesive text description
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account profile for this institution/group entity, faithfully reproducing real-world situations as closely as possible.
...
2. persona: Detailed account profile description (2000-character plain text), including:
   ...
   - Stance and attitude (official position on core topics, how to handle controversy)
...
- persona must be a continuous, cohesive text description with no newlines
- {get_language_instruction()} (gender field must use English "other")
- age must be the integer 30; gender must be the string "other"
- The account's communications must be consistent with its institutional identity"""
    ...
    def set_graph_id(self, graph_id: str):
        """Set the knowledge graph ID for Zep retrieval"""
        self.graph_id = graph_id
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            graph_id: Knowledge graph ID for Zep retrieval to obtain richer context
            parallel_count: Number of parallel generations, default 5
            realtime_output_path: File path for real-time output (if provided, writes after each profile is generated)
        ...
        completed_count = [0]  # Use a list so it can be modified inside a closure
        lock = Lock()
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        - description: Displayed externally, visible to other users in the bio
        """
        import csv
    ...
    def _normalize_gender(self, gender: Optional[str]) -> str:
        ...
        # Chinese-to-English mapping (and English synonyms)
        gender_map = {
            "male": "male",
            "female": "female",
            "man": "male",
