...
Wraps knowledge graph search, node reading, edge querying, and related tools for use by the Report Agent
...
2. PanoramaSearch (broad search) - gets the full picture, including expired/historical content
    ...
    def to_text(self) -> str:
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                if skip:
                    continue
                # Truncate overly long content (truncate at sentence boundary, not hard cut)
...
class ZepToolsService:
    ...
    4. interview_agents - in-depth interviews (interviews simulated Agents, gathers multi-perspective views)

    [Basic tools]
    - search_graph - knowledge graph semantic search
    - get_all_nodes - fetch all nodes from the graph
    - get_all_edges - fetch all edges from the graph (with temporal information)
    - get_node_detail - get detailed information for a node
    - get_node_edges - get edges associated with a node
    ...
    def get_all_nodes(self, graph_id: str) -> List[NodeInfo]:
        """
        Fetch all nodes from the graph (with pagination)
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        """
        Fetch all edges from the graph (with pagination, including temporal information)
    ...
    def get_node_edges(self, graph_id: str, node_uuid: str) -> List[EdgeInfo]:
        ...
        Fetches all edges in the graph and filters for those connected to the specified node.
    ...
    def get_simulation_context(
        ...
        Performs a comprehensive search for all information relevant to the simulation requirement.
        ...
        # Filter for entities with actual custom types (non-pure Entity nodes)
        entities = []
        for node in all_nodes:
    ...
    def insight_forge(
        ...
        # Fetch details for all relevant entities (no count limit, full output)
        entity_insights = []
...
Return a JSON-formatted list of sub-questions."""
    ...
    def panorama_search(
        ...
        2. Fetches all edges (including expired/invalid ones)
        ...
        for edge in all_edges:
            ...
            # Determine whether the edge is expired/invalid
    ...
    def interview_agents(
        ...
        1. Automatically reads the profile file to understand all simulated Agents
        ...
        [Use cases]
        - Need to understand perspectives on an event from different roles
        ...
        Args:
            simulation_id: Simulation ID (used to locate the profile file and call the interview API)
        ...
        # Step 2: Use LLM to select Agents to interview (returns a list of agent_ids)
        ...
        # Add an optimizing prefix to constrain the Agent's response format
        INTERVIEW_PROMPT_PREFIX = (
            ...
            "5. Separate your answers for each question with a blank line.\n"
            "6. Provide substantive content — answer each question with at least 2–3 sentences.\n\n"
        )
        optimized_prompt = f"{INTERVIEW_PROMPT_PREFIX}{combined_prompt}"

        # Step 4: Call the real interview API (no platform specified — both platforms interviewed simultaneously by default)
        try:
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                # Strip any JSON tool-call wrappers from responses
                ...
                # Clean response text: remove labels, numbering, Markdown, and other noise
                clean_text = re.sub(r'#{1,6}\s+', '', combined_responses)
                clean_text = re.sub(r'\{[^}]*tool_name[^}]*\}', '', clean_text)
                clean_text = re.sub(r'[*_`|>~\-]{2,}', '', clean_text)
                clean_text = re.sub(r'(?:问题|[Qq]uestion)\s*\d+\s*[：:]\s*', '', clean_text)
                ...
                meaningful = [
                    ...
                    and not s.strip().startswith(('{', '问题', 'Question', 'question'))
                ]
                meaningful.sort(key=len, reverse=True)
                key_quotes = [s + "。" for s in meaningful[:3]]

                # Strategy 2 (supplemental): correctly paired Chinese 「」 quotation marks containing longer text
                if not key_quotes:
            ...
            result.summary = f"Interview failed: {str(e)}. The simulation environment may have shut down — please ensure the OASIS environment is running."
            return result
        except Exception as e:
    ...
    def _load_agent_profiles(self, simulation_id: str) -> List[Dict[str, Any]]:
        """Load the Agent profile file for the simulation"""
        import os
        import csv
        ...
        # Fall back to reading the Twitter CSV format
    ...
    def _select_agents_for_interview(
        ...
        system_prompt = """You are a professional interview planning expert. Your task is to select the most suitable interview subjects from a list of simulated Agents based on the interview requirement.
...
3. Select a diverse range of viewpoints (e.g., supporters, opponents, neutral parties, subject-matter experts, etc.).
4. Prioritize roles directly related to the event.

Return JSON format:
{
    "selected_indices": [list of selected Agent indices],
    "reasoning": "explanation of the selection reasoning"
}"""
...
Please select up to {max_agents} Agents most suitable for this interview, and explain your reasoning."""
        ...
        try:
            ...
            # Fetch complete information for selected Agents
            selected_agents = []
            valid_indices = []
...
2. Questions that may elicit different answers depending on the role.
...
6. Ask directly — do not include background context or prefixes.
...
1. Distill the main perspectives from each party.
