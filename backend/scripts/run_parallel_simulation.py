...
- After the simulation completes, the environment is not immediately closed; it enters command-waiting mode
...
# This is to fix the issue where the OASIS third-party library reads files
...
if sys.platform == 'win32':
    ...
    # Reconfigure standard output streams to UTF-8 (fixes garbled characters in the console)
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None,
        ...
        This fixes the issue where third-party libraries (such as OASIS) read files
        without specifying an encoding.
        """
...
# Add the filter immediately at module load time, before camel code executes
...
def disable_oasis_logging():
    ...
    OASIS logs are too verbose (they record each agent's observations and actions);
...
def init_logging_for_simulation(simulation_dir: str):
    """
    Initialize log configuration for the simulation
...
class ParallelIPCHandler:
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        if results:
            ...
            self.send_response(command_id, "failed", error="No interviews completed successfully")
            return False
...
def fetch_new_actions_from_db(
    ...
    Args:
        ...
        last_rowid: Maximum rowid from the last read (using rowid instead of created_at,
    ...
    try:
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Simplify action_args, retaining only key fields (keep full content, no truncation)
            simplified_args = {}
...
def _enrich_action_context(
    ...
    """
    try:
        # Like/dislike a post: enrich with post content and author
        ...
        # Quote post: enrich with original post content, author, and quote comment
            ...
            # Get the quote comment content (quote_content) of the quoting post
            if new_post_id:
                cursor.execute("""
        ...
        # Follow a user: enrich with the followed user's name
        ...
        # Mute a user: enrich with the muted user's name
        ...
        # Like/dislike a comment: enrich with comment content and author
        ...
        # Create a comment: enrich with information about the commented post
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    during parallel simulation, increasing concurrency capacity.
    ...
    # Choose which LLM to use based on the parameter and available configuration
    ...
    # Fall back to config if .env does not have the model name
    if not llm_model:
...
async def run_twitter_simulation(
    ...
    # Get real Agent name mappings from the config file (using entity_name instead of the default Agent_X)
    agent_names = get_agent_names_from_config(config)
    # For any agent not in the config, use OASIS's default name
...
async def main():
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Close the environment immediately after simulation completes, do not enter command-waiting mode'
    )
...
def setup_signal_handlers(loop=None):
    """
    Set up signal handlers to ensure the process exits correctly when SIGTERM/SIGINT is received.

    Persistent simulation scenario: the program does not exit after the simulation completes;
    it waits for interview commands. When a termination signal is received:
